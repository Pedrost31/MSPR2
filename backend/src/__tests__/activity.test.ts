...
describe('GET /api/activities', () => {
  ...
  it('filtre par type', async () => {
    const res = await request(app).get('/api/activities?type=cardio').set(auth());
    expect(res.status).toBe(200);
    res.body.data.entries.forEach((a: { type: string }) => expect(a.type).toBe('cardio'));
  });
});

describe('GET /api/activities/weekly-summary', () => {
  it('retourne le résumé de la semaine', async () => {
    const res = await request(app).get('/api/activities/weekly-summary').set(auth());
    expect(res.status).toBe(200);
...
describe('DELETE /api/activities/:id', () => {
  it('supprime l\'activité', async () => {
    const del = await request(app).delete(`/api/activities/${activityId}`).set(auth());
    expect(del.status).toBe(200);

    const get = await request(app).get(`/api/activities/${activityId}`).set(auth());
