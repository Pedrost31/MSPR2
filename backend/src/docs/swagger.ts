...
const options: swaggerJSDoc.Options = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'API REST pour la gestion de la santé, nutrition et activités physiques avec recommandations IA. ' +
        'Authentification via JWT Bearer token.',
      contact: { name: 'HealthAI Team', email: 'contact@healthai.com' },
    },
    servers: [{ url: `http://localhost:${env.PORT}/api`, description: 'Développement local' }],
    components: {
      securitySchemes: {
